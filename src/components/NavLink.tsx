...
const VARIANT_STYLES: Record<NavVariant, { base: string; active: string; inactive: string }> = {
  desktop: {
    base: 'flex items-center space-x-2 px-3 py-1.5 text-sm font-medium transition-all',
    active: 'border-b-2 border-primary text-primary',
    inactive: 'text-secondary hover:text-on-surface',
  },
  'mobile-tab': {
    base: 'flex flex-col items-center space-y-1 text-[10px] font-medium transition-all',
    active: 'text-primary',
  ...
  'mobile-drawer': {
    ...
    inactive: 'border-outline-variant/60 bg-surface-container-low hover:border-outline text-secondary hover:text-on-surface',
  },
};

const badgeOverlaysIcon = (variant: NavVariant) => variant !== 'mobile-drawer';

export const NavLink = forwardRef<HTMLButtonElement, NavLinkProps>(function NavLink(
  { tab, label, icon, badge, active, variant, controlsId, className, onClick, ...rest },
  ref,
) {
  ...
  return (
    <button
      ...
      {...rest}
    >
      <span className={cn('flex items-center', variant === 'mobile-tab' ? 'flex-col space-y-1' : 'space-x-2')}>
        {overlay ? (
          <span className="relative inline-flex">
            <span aria-hidden="true" className="shrink-0">{icon}</span>
            <span className="absolute -top-1.5 -right-2 flex items-center justify-center">{badge}</span>
          </span>
        ) : (
          <span className="relative inline-flex">
            <span aria-hidden="true" className="shrink-0">{icon}</span>
