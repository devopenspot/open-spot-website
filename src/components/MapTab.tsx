...
export default function MapTab({
  ...
  const filteredSpots = useMemo(
    () => (selectedType === 'All' ? spots : spots.filter(s => s.type === selectedType)),
    [spots, selectedType],
  );
  ...
  const handleTypeSelect = useCallback((type: string) => setSelectedType(type), []);
    ...
    >
        ...
        >
          {filteredSpots.map(spot => {
            ...
            return (
              ...
              >
                <span className="h-12 w-12 rounded-lg bg-black overflow-hidden shrink-0">
                  <img
                    src={spot.image}
                    alt=""
      ...
      >
        ...
        >
          ...
          <span className="flex items-center space-x-1.5">
            <span aria-hidden="true" className="h-2 w-2 rounded-full bg-[#eeeeee] border border-outline" />
