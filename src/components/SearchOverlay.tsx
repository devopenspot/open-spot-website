    ...
    >
      <div className="mx-auto w-full max-w-3xl">
        <div className="flex items-center justify-between border-b border-outline-variant pb-4 mb-6">
          <h2 id={titleId} className="visually-hidden">Search spots</h2>
          <div className="flex items-center space-x-3 flex-1 mr-4">
            <label htmlFor={inputId} className="visually-hidden">
              Search spots by name, city, style, or features
            </label>
            <Search size={20} className="text-secondary shrink-0" aria-hidden="true" />
            <input
              ref={inputRef}
              id={inputId}
              type="search"
          ...
          >
            <span>Close</span>
            <kbd aria-hidden="true" className="hidden md:inline rounded bg-surface-container px-1 py-0.5 text-[9px] font-mono">
              ESC
            </kbd>
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              <div>
                <div className="flex items-center space-x-2 mb-4">
                  <History size={14} className="text-secondary" aria-hidden="true" />
                ...
                <ul className="space-y-2">
                  {RECENT_SEARCHES.map(term => (
                    <li key={term}>
                      ...
                      >
                        <span>{term}</span>
                        <ArrowRight size={12} className="text-secondary" aria-hidden="true" />
                      </button>
                    </li>
                  ))}
                </ul>
              ...
              <div>
                <div className="flex items-center space-x-2 mb-4">
                  <Compass size={14} className="text-secondary" aria-hidden="true" />
                  <h3 className="font-mono text-[10px] font-bold tracking-widest text-secondary uppercase">
                    Popular queries
                  </h3>
                </div>
            ...
            <div className="space-y-4">
              ...
              {filteredSpots.length > 0 ? (
                <ul className="space-y-3" aria-label="Matched spots">
                  {filteredSpots.map(spot => (
                    <li key={spot.id}>
                      ...
                      >
                        <span className="h-16 w-16 rounded-lg bg-black overflow-hidden shrink-0">
                          <img
                            src={spot.image}
                            alt=""
                        ...
                        <span className="flex-1 min-w-0">
                          ...
                          <span className="text-xs text-secondary flex items-center mt-1">
                            <MapPin size={11} className="mr-0.5 shrink-0" aria-hidden="true" />
                            {spot.city} • <span className="ml-1 italic">&ldquo;{spot.features.slice(0, 2).join(', ')}&rdquo;</span>
                          </span>
                        </span>

                        <ArrowRight size={16} className="text-secondary shrink-0" aria-hidden="true" />
                      </button>
                    </li>
                  ))}
                </ul>
                ...
                <div className="py-12 text-center text-xs font-mono text-secondary rounded-xl border border-dashed border-outline-variant">
                  No plots detected for &ldquo;{deferredQuery.toUpperCase()}&rdquo;
                </div>
              )}
            </div>
          )}
