...
export function classifySpotType(raw: NominatimResponse): string {
  ...
  if (
    /\bskatepark\b|\bskate_park\b|\bskateboard\b|\bskate-park\b/.test(haystack)
  ) {
    return "skatepark";
  }
  if (/\bpark\b|\bpitch\b|\bplayground\b/.test(haystack)) return "skatepark";
  if (/\bplaza\b|\bsquare\b/.test(haystack)) return "plaza";

  // Class-based fallbacks.
  if (cls === "leisure") return "skatepark";
