...
- **Connection strategy: single source = `SUPABASE_DATABASE_URL`.** Local dev and Vercel set the same env var to the same value (the same Supabase project). `getDatabaseUrl()` resolves `SUPABASE_DATABASE_URL` (pooler, port 6543) → `SUPABASE_DIRECT_URL` (port 5432, fallback) → `DB_CONNETION_STRING` (legacy typo, CI only) → `DATABASE_URL` (optional escape hatch) → `null`. The build never calls this — it is DB-free. The `postgres-js` client config (`prepare: false`, `ssl: "require"`, `max: 5`, `idle_timeout: 30`, `connect_timeout: 8`, `max_lifetime: 1800`) is compatible with pgBouncer **Transaction** mode. **Use the Transaction mode pooler URL, not the Session mode pooler URL** — they share the same host and port but Session mode holds a server connection for the entire client session and saturates fast under serverless concurrency (error: `EMAXCONNSESSION ... max clients reached in session mode`).
...
- **Postgres connection uses `ssl: "require"`** in `src/lib/db/client.ts` and `apply-sql.ts`. The Supabase **Transaction** pooler URL (`aws-0-<region>.pooler.supabase.com:6543`, mode: Transaction) and the direct URL (`db.<project>.supabase.co:5432`) both work. The Session mode pooler URL (`aws-0-<region>.pooler.supabase.com:6543`, mode: Session) is **not** supported — it saturates at `pool_size: 15` and throws `EMAXCONNSESSION` under serverless concurrency. No local `docker-compose.yml` is checked in — `docker compose up -d infra-db` mentioned in `.env.example` requires you to bring the compose file yourself; ask before assuming infra is available.
