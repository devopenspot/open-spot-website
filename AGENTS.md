...
- **Build is DB-free. The runtime reads exclusively from Postgres via Drizzle.** `SUPABASE_DIRECT_URL` (port 5432, direct) is the single source of connection — set in both `.env.local` (local dev) and Vercel. Dev and prod share the same Supabase project, so the string is the same in both. Local docker Postgres is no longer in the runtime path.
...
- **Connection strategy: single source = `SUPABASE_DIRECT_URL`.** Local dev and Vercel set the same env var to the same value (the same Supabase project). No docker default, no pooler. `getDatabaseUrl()` resolves `SUPABASE_DIRECT_URL` → `DB_CONNETION_STRING` (legacy typo, CI only) → `DATABASE_URL` (optional escape hatch) → `null`. The build never calls this — it is DB-free.
...
- **Direct Postgres URL only.** `SUPABASE_DIRECT_URL` is port 5432 (direct). The pooled `SUPABASE_DATABASE_URL` (port 6543) is documented in `.env.example` but ignored by the runtime today. `getDatabaseUrl()` resolution order in `src/lib/env.ts`: `SUPABASE_DIRECT_URL` → `DB_CONNETION_STRING` → `DATABASE_URL` → `null`. The dev-console scripts (`db:seed`, `db:apply`, `db:health`) read through the same `getDatabaseUrl()`.
