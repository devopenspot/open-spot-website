...
- **Postgres is required to run dev and build.** There is no JSON read fallback. Bring a local docker container or point at Supabase. The runtime reads exclusively from the DB via Drizzle.
...
- The runtime reads through `SUPABASE_DIRECT_URL` port 5432 — *not* the pgBouncer 6543 pooled URL.
...
- **Dimension reads are DB-only.** `src/data.ts` (`REGIONS_DATA`, `COUNTRY_TO_REGION`, `COUNTRY_NAME_OVERRIDES`, `DEFAULT_PRESET_IMAGES`) is the **seed bootstrap only** — it seeds `regions` / `countries` / `spot_types` / `sport_disciplines` / `event_tiers` via `src/db/seed.ts`. At runtime, the spot/event repositories read dimension tables directly. The `Region[]` consumed by client components is assembled server-side by `src/lib/data/regions.ts` (`getRegionsForClient()`, called from `RootDataProviders`) and delivered to the client tree via the `useSpotsStore` Zustand store (hydrated by `SpotsProvider`). `useMapFilter`, `RegionFilter`, and `ExploreTab` all read regions from `useSpotsStore((s) => s.regions)`. `DEFAULT_PRESET_IMAGES` stays in `src/data.ts` as pure UI config (not in the DB; the Drizzle spot repo depends on it for `pickFallbackImage`).
...
- Static data (seed bootstrap + UI config): `src/data.ts` + `src/data/spots.json` + `src/data/sport-events.json` — **seed bootstrap only** (consumed by `src/db/seed.ts`). The `Region` constants in `src/data.ts` are also referenced by the seed. `DEFAULT_PRESET_IMAGES` is UI config used by the Drizzle spot repo's `pickFallbackImage` and the admin `ImageSourceField` client component. At runtime, all dimension reads come from the DB.
